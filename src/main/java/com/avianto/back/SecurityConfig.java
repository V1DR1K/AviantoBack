...
@Component class JwtFilter extends OncePerRequestFilter {
  private final JwtService jwt; private final DataRepository db; JwtFilter(JwtService jwt, DataRepository db){this.jwt=jwt;this.db=db;}
  @Override protected void doFilterInternal(HttpServletRequest request,HttpServletResponse response,FilterChain chain)throws ServletException,IOException {String header=request.getHeader("Authorization");if(header!=null&&header.startsWith("Bearer "))try{Claims c=jwt.claims(header.substring(7));UUID id=UUID.fromString(c.getSubject());AppUser user=db.one("select u from AppUser u where u.id=:id and u.activo=true and u.deletedAt is null",AppUser.class,Map.of("id",id));String role=c.get("role",String.class);if(user!=null&&user.rol.name().equals(role))SecurityContextHolder.getContext().setAuthentication(new UsernamePasswordAuthenticationToken(user.id.toString(),null,List.of(new SimpleGrantedAuthority("ROLE_"+user.rol.name()))));}catch(Exception ignored){}chain.doFilter(request,response);}
}
