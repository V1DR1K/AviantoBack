...
public class AuthService {
  ...
  public SessionResponse refresh(RefreshRequest r){RefreshToken t=db.oneForUpdate("select t from RefreshToken t where t.tokenHash=:hash",RefreshToken.class,Map.of("hash",hash(r.refreshToken())));if(t==null||t.revokedAt!=null||t.expiresAt.isBefore(Instant.now())||!t.user.activo||t.user.deletedAt!=null)throw new BusinessException(401,"Sesión inválida");t.revokedAt=Instant.now();return session(t.user);}
  public void logout(RefreshRequest r){RefreshToken t=db.oneForUpdate("select t from RefreshToken t where t.tokenHash=:hash",RefreshToken.class,Map.of("hash",hash(r.refreshToken())));if(t!=null)t.revokedAt=Instant.now();}
