...
@Entity class Motovehiculo extends BaseEntity { @ManyToOne(fetch=FetchType.LAZY) Cliente cliente; @ManyToOne(fetch=FetchType.LAZY) @JoinColumn(name="marca_id") MarcaMoto marca; String modelo; String patente; Integer anio; Integer kilometraje; String color; String cilindrada; @Column(columnDefinition="text") String observaciones; boolean activo=true; }
@Entity @Table(name="item_catalogo") class ItemCatalogo extends BaseEntity { String descripcion; @Enumerated(EnumType.STRING) ItemType tipo; @ManyToOne(fetch=FetchType.LAZY) @JoinColumn(name="categoria_id") CategoriaCatalogo categoria; @Column(name="precio_base") BigDecimal precioBase; @Column(columnDefinition="text") String observaciones; boolean activo=true; }
...
@Entity @Table(name="auditoria") class Auditoria { @Id UUID id=UUID.randomUUID(); Instant fecha=Instant.now(); @ManyToOne(fetch=FetchType.LAZY) @JoinColumn(name="usuario_id") AppUser usuario; String modulo; String accion; @Column(columnDefinition="text") String descripcion; }
